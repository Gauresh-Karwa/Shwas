...
def get_nearby_fires(map_key: str, center_lat: float, center_lon: float, radius_deg: float=0.5, sensor: str='VIIRS_SNPP_NRT', days: int=1) -> list[FireDetection]:
    ...
    for row in reader:
        ...
        acquired_at = datetime.strptime(f"{row['acq_date']} {row['acq_time']}", '%Y-%m-%d %H%M')
        detections.append(FireDetection(latitude=lat, longitude=lon, distance_km=round(distance, 1), acquired_at=acquired_at, frp_mw=frp, confidence=confidence))
