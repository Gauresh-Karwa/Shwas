...
SYSTEM_PROMPT = """You explain air quality to everyday people in India in \
ONE short, natural, conversational sentence - like a friendly weather app \
notification, NOT a report. You will be given fixed facts: a station name, \
a computed AQI value and category, the dominant pollutant, wind \
conditions, nearby fire detections, and relevant recent news.

Rules you must follow exactly:
- Use ONLY the facts provided. Never invent a cause not supported by them.
- Never state a cause as certain. Use hedged language: "likely", "may be \
linked to", "could be contributing".
- Never state or imply an AQI value or category different from the one given.
- If a factor (wind, fire, news) has no data, simply don't mention it — \
don't say "no data" or apologize for missing it.
- ONE sentence only. No line breaks. No paragraphs. Under 25 words.
- NEVER use phrases like "the AQI is currently", "falls into the category", \
"the dominant pollutant contributing to this reading is", or any similarly \
clinical/report-style wording.
...
def _build_user_prompt(
    station_name: str, aqi_value: int, category: str, dominant_pollutant: str,
    wind: WindData | None, fires: list[FireDetection], news: list[NewsResult],
) -> str:
    lines = [
        f"Station: {station_name}",
        f"Computed AQI: {aqi_value} ({category})",
        f"Dominant pollutant: {dominant_pollutant}",
    ]
    if wind:
        lines.append(
            f"Wind: {wind.speed_mps} m/s from {compass_direction(wind.direction_deg)}, "
            f"conditions: {wind.description}"
        )
    if fires:
        nearest = fires[0]
