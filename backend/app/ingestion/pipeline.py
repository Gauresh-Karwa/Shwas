...
def _group_by_station_and_time(records: list) -> dict:
    ...
    for r in records:
        ...
        grouped[station_id, timestamp, r['station']][r['pollutant_id']] = (r['min_value'], r['max_value'], r['avg_value'])
    return grouped
...
def run_ingestion_cycle(db=None, records=None) -> dict:
    ...
    if db is None:
        db = SessionLocal()
    if records is None:
        records = fetch_city_data(settings.CPCB_BASE_URL, settings.CPCB_RESOURCE_ID, settings.CPCB_API_KEY, settings.TARGET_CITY)
    grouped = _group_by_station_and_time(records)
    summary = {'stations_processed': 0, 'stations_skipped_unknown': 0, 'raw_readings_written': 0, 'raw_readings_duplicate_skipped': 0, 'aqi_ok': 0, 'aqi_insufficient': 0}
    for (station_id, timestamp, station_name), pollutants in grouped.items():
        ...
        for pollutant_id, (raw_min, raw_max, raw_avg) in pollutants.items():
            ...
            db.add(RawReading(station_id=station_id, pollutant_id=pollutant_id, min_value=cleaned.min_value if not cleaned.is_corrected else None, max_value=cleaned.max_value if not cleaned.is_corrected else None, avg_value=cleaned.avg_value, timestamp=timestamp))
            db.add(CleanedReading(station_id=station_id, pollutant_id=pollutant_id, min_value=cleaned.min_value, max_value=cleaned.max_value, avg_value=cleaned.avg_value, timestamp=timestamp, is_corrected=cleaned.is_corrected, correction_reason=cleaned.correction_reason))
        ...
        db.add(StationAQI(station_id=station_id, timestamp=timestamp, aqi_value=result.aqi, status=result.status, category=result.category, dominant_pollutant=result.dominant_pollutant, sub_indices=result.sub_indices, reason=result.reason))
