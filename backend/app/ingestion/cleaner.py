    ...
    for name, val in [("min_value", min_value), ("max_value", max_value), ("avg_value", avg_value)]:
        if val is not None and val < 0:
            return CleanedValue(
                ...
                is_corrected=True,
                correction_reason=f"Rejected: negative {name} ({val}) is physically invalid.",
            )
    ...
    if min_value is not None and max_value is not None and min_value > 0:
        ...
        if ratio > SPREAD_RATIO_THRESHOLD:
            return CleanedValue(
                ...
                is_corrected=True,
                correction_reason=f"High variance flagged: max/min ratio {ratio:.1f}x "
                                   f"exceeds {SPREAD_RATIO_THRESHOLD}x threshold "
