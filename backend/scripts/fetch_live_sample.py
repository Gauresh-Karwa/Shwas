...
def fetch_page(url: str, params: dict) -> dict:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            last_error = e
            print(f'  Attempt {attempt}/{MAX_RETRIES} failed ({type(e).__name__}), retrying in {RETRY_BACKOFF_SECONDS}s...')
...
def fetch_city_data(city: str) -> dict:
    ...
    final_payload = None
    while True:
        params = {'api-key': settings.CPCB_API_KEY, 'format': 'json', 'limit': PAGE_SIZE, 'offset': offset, 'filters[city]': city}
