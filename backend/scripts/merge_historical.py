...
KAGGLE_TO_LIVE_STATION_ID = {
    "MH006": "borivali-east-mumbai-mpcb", 
    "MH007": "chhatrapati-shivaji-intl-airport-t2-mumbai-mpcb",
    "MH009": "kurla-mumbai-mpcb",
    "MH010": "powai-mumbai-mpcb",
    "MH011": "sion-mumbai-mpcb",
    "MH014": "worli-mumbai-mpcb",
}

POLLUTANT_COLUMN_MAP = {
    "PM2.5": "PM2.5", "PM10": "PM10", "NO2": "NO2",
    "NH3": "NH3", "CO": "CO", "SO2": "SO2", "O3": "OZONE",
}
    ...
    for _, row in df.iterrows():
        ...
        for kaggle_col, pollutant_id in POLLUTANT_COLUMN_MAP.items():
            ...
            db.add(RawReading(
                ...
                min_value=float(value), max_value=float(value), avg_value=float(value),
                timestamp=timestamp,
            ))
            raw_written += 1
        ...
        if pd.isna(aqi_value):
            db.add(StationAQI(
                station_id=station_id, timestamp=timestamp,
                aqi_value=None, status="insufficient_data",
                reason="Kaggle historical row had no AQI value for this hour.",
            ))
        else:
            db.add(StationAQI(
                station_id=station_id, timestamp=timestamp,
